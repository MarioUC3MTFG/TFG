...
# Primero definimos la función de Markowitz que posteriormente llamaremos en cada rebalanceo del bucle. 
def pesos_markowitz(retornos):
    ...
    # Restricción: Ponderación máxima por activo <= 0.10
    # Esto se formula como I * w <= b_upper, donde I es la matriz identidad
    # y b_upper es un vector de 0.10s.
    A_upper = np.identity(n_assets)
    b_upper = np.full(n_assets, 0.10)
    port.add_constraint(A=A_upper, B=b_upper)

    pesos = port.optimization(model = 'Classic', rm = 'MV', obj = 'Sharpe', rf = 0)
...
def backtest_markowitz(retornos, lookback = 756):
    ...
    for date in retornos.index[lookback:]:
        if ponderaciones is None or date in fechas_rebalanceo:
            ...
            ponderaciones = pesos_markowitz(Historico)      
...
rentabilidad_cartera = backtest_markowitz(retornos)


print("\n--- Métricas de la cartera de Markowitz ---")
