...
# Primero definimos la función de Hierarchical risk parity que posteriormente llamaremos en cada rebalanceo del bucle. 
def pesos_HRP(retornos):
    port = rp.HCPortfolio(returns = retornos)
    pesos = port.optimization(model = 'HERC', codependence = 'pearson', rm = 'MV', rf = 0, linkage = 'average', max_k = 10   , leaf_order = True)
...
def backtest_HRP(retornos, lookback = 756):
    ...
    for date in retornos.index[lookback:]:
        if ponderaciones is None or date in fechas_rebalanceo:
            ...
            ponderaciones = pesos_HRP(Historico)      
...
rentabilidad_cartera = backtest_HRP(retornos)


print("Métricas de la cartera de Hierarchical risk parity")
